...
public class BlockBasicCeres extends Block implements IDetectableResource, IPlantableBlock, ITerraformableBlock, ISortableBlock {
	public static final PropertyEnum<EnumBlockBasic> BASIC_TYPE = PropertyEnum.create("basictypeceres", EnumBlockBasic.class);

	public enum EnumBlockBasic implements IStringSerializable {
		SURFACE(0, "ceres_surface"), SUB_SURFACE(1, "ceres_sub_surface"), STONE(2, "ceres_stone"), ORE_IRON(3, "ceres_ore_iron"), ORE_TIN(4, "ceres_ore_tin"), ORE_COPPER(5, "ceres_ore_copper"), ORE_URANIUM(6, "ceres_ore_uranium"), URANIUM_BLOCK(7,
				"ceres_uranium_block"), STONEBRICKS(8, "ceres_stonebricks");
	...
	public float getExplosionResistance(World world, BlockPos pos, Entity exploder, Explosion explosion) {
		...
		if (state.getValue(BASIC_TYPE) == EnumBlockBasic.STONE || state.getValue(BASIC_TYPE) == EnumBlockBasic.STONEBRICKS)
			return 6.0F;
		else if (state.getValue(BASIC_TYPE) == EnumBlockBasic.ORE_COPPER || state.getValue(BASIC_TYPE) == EnumBlockBasic.ORE_IRON || state.getValue(BASIC_TYPE) == EnumBlockBasic.ORE_TIN || state.getValue(BASIC_TYPE) == EnumBlockBasic.ORE_URANIUM)
			return 3.0F;
